...
class RegistrationRepository {
  ...
  Future<List<Registration>> loadRegistration() async {
    Response response = await NetworkProvider()
        .get("http://nibrahim.pythonanywhere.com/registration/");
  ...
  Future<RegistrationDetailsResponse> loadRegistrationDetail(int id) async {
    Response response = await NetworkProvider()
        .get("http://nibrahim.pythonanywhere.com/registration/$id");
    switch (response.statusCode) {
      case 200:
  ...
  Future<String> loadRegistrationDelete(
    ...
    Response response = await NetworkProvider()
        .delete("http://nibrahim.pythonanywhere.com/registration/$id", data: {
      "student": studentId,
      "subject": subjectId,
    });
    switch (response.statusCode) {
